...
const WatchBattleEnemyPlayer = ({type}) => {
    const watchBattle = useSelector((state) => state.watchBattle)

    const playerClass = watchBattle ? watchBattle.player2?.playerClass : null
    const armorFull = []

    const stats = watchBattle ? Object.entries(watchBattle.player2?.total_stats).slice(6, 10) : null
